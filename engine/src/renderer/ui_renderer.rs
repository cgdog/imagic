...
use crate::behaviors::behavior_wrapper::BehaviorWrapper;
...
impl UIRenderer {
    ...
    pub(crate) fn draw(
        ...
        let window= graphics_context.main_window().get_ref();
        let device: &Device = &graphics_context.device;
        let queue: &Queue = &graphics_context.queue;

        let mut screen_descriptor = ScreenDescriptor {
            size_in_pixels: [graphics_context.surface_config.width, graphics_context.surface_config.height],
        ...
        let full_output = self.state.egui_ctx().run(raw_input, |ctx| {
            for behavior_wrapper in &mut *behavior_wrappers {
                behavior_wrapper.on_gui(logic_context, ctx);
            }
        });
