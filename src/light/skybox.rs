...
impl Skybox {
    ...
    pub fn init_with_cube_texture(&mut self, imagic_context: &mut ImagicContext, background_texture: ID) {
        let mut skybox_material = Box::new(SkyboxMaterial::new());
        skybox_material.set_skybox_map(background_texture);
        skybox_material.set_cull_mode(wgpu::Face::Front);
        let skybox_material_id = imagic_context
            .material_manager_mut()
            .add_material(skybox_material);
        ...
        self.background_texture_id = background_texture;
        self.skybox_material_id = skybox_material_id;
    }
}
