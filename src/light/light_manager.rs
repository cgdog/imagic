...
impl LightManager {
    ...
    pub fn init_after_app(&mut self, graphics_context: &GraphicsContext, bind_group_manager: &mut BindGroupManager
        ...
        self.create_bind_group(graphics_context, bind_group_manager, bind_group_layout_manager);
    }
    ...
    fn create_light_buffer(&mut self, graphics_context: &GraphicsContext, transform_manager: &TransformManager) {
        ...
        let lights_storage_buffer = graphics_context.create_buffer_init(&wgpu::util::BufferInitDescriptor {
    ...
    pub fn get_binding_resource(&self) -> Option<wgpu::BindingResource> {
        match &self.light_buffer {
            Some(light_storage_buffer) => {
                Some(light_storage_buffer.as_entire_binding())
            },
            _ => None
        }
    }
    ...
    fn create_bind_group(&mut self, graphics_context: &GraphicsContext, bind_group_manager: &mut BindGroupManager
        , bind_group_layout_manager: &mut BindGroupLayoutManager) -> usize {
    ...
    // pub fn get_bind_group_layout_id(&self) -> usize {
