use log::info;
use wgpu::{util::DeviceExt, Adapter, BindGroup, BindGroupLayout, BindGroupLayoutDescriptor, CommandBuffer, CommandEncoder, Device, Instance, PipelineLayout, Queue, RenderPipeline, ShaderModule, SubmissionIndex, TextureFormat};
...
impl GraphicsContext {
    ...
    pub async fn init(&mut self, window: &Window) {
        let surface = self.instance.create_surface(window.get()).expect("Instance failed to create surface.");
    ...
    pub fn get_swapchian_format(&self) -> TextureFormat {
        let swapchain_capabilities = self.surface.get_capabilities(self.adapter.as_ref().unwrap());
        let swapchain_format = swapchain_capabilities.formats[0];
        swapchain_format
    }

    pub fn create_shader_module(&self, shader_module_descriptor: wgpu::ShaderModuleDescriptor) -> ShaderModule {
        self.get_device().create_shader_module(shader_module_descriptor)
    }

    pub fn create_pipeline_layout(&self, pipeline_layout_desc: &wgpu::PipelineLayoutDescriptor) -> PipelineLayout {
        self.get_device().create_pipeline_layout(pipeline_layout_desc)
    }

    pub fn create_render_pipeline(&self, render_pipeline_desc: &wgpu::RenderPipelineDescriptor) -> RenderPipeline {
        self.get_device().create_render_pipeline(render_pipeline_desc)
    }
    ...
    pub fn create_command_encoder(&self, cmd_encoder_desc: &wgpu::CommandEncoderDescriptor) -> CommandEncoder {
    ...
    pub fn create_buffer_init(&self, buffer_desc: &wgpu::util::BufferInitDescriptor) -> wgpu::Buffer {
